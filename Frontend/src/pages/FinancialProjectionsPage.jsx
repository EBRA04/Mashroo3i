...
const SLIDER_CSS = `
  ...
  /* ─── Print overrides ──────────────────────────────────────────────────── */
  @media print {
    /* Hide wizard navigation: progress bar, back/export buttons */
    .fp-no-print { display: none !important; }

    /* Header breadcrumb + title: keep but compress */
    .fp-print-header { margin-bottom: 0.5rem !important; }

    /* Cards: no shadows, break cleanly across pages */
    .fp-card {
    ...
    /* KPI grid: 2 columns on paper to save vertical space */
    .fp-kpi-grid {
      grid-template-columns: repeat(2, 1fr) !important;
    }

    /* Chart: allow it to break if it's tall */
    .fp-chart-wrapper {
      page-break-inside: auto;
      break-inside: auto;
    }

    /* Print title injected at top */
    .fp-print-title {
...
// ─── Step 3 — Results ─────────────────────────────────────────────────────────
function Step3({ inputs, projections, onBack, ideaId, sectorLabel, onReportReady }) {
    ...
    async function load() {
      ...
      try {
        ...
        if (!cancelled && Array.isArray(data) && data.length > 0) {
          setInsights(data)
          onReportReady?.({ inputs, projections, insights: data, sectorLabel })
        } else {
          const fallback = buildFallbackInsights({ projections, ...inputs })
          if (!cancelled) { setInsights(fallback); onReportReady?.({ inputs, projections, insights: fallback, sectorLabel }) }
        }
      } catch {
        const fallback = buildFallbackInsights({ projections, ...inputs })
        if (!cancelled) { setInsights(fallback); onReportReady?.({ inputs, projections, insights: fallback, sectorLabel }) }
  ...
  return (
    <>
      {/* Print-only title — hidden in browser, shown on paper */}
      <div style={{ display: 'none' }} className="fp-print-title">
        Financial Projections — Year 1 Report
      </div>
      ...
      <Card style={{ marginBottom: '1rem' }}>
        ...
        <div className="fp-kpi-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(180px, 1fr))', gap: '0.75rem', marginBottom: '1.25rem' }}>
      ...
      <div className="fp-chart-wrapper" style={{ marginBottom: '1rem' }}>
      ...
      <div className="fp-no-print" style={{ display: 'flex', justifyContent: 'flex-start', gap: '0.625rem', flexWrap: 'wrap', background: '#fff', border: `1px solid ${C.n200}`, borderRadius: '0.875rem', boxShadow: '0 1px 3px rgba(0,0,0,0.04)', padding: '1rem 1.25rem' }}>
        <BtnSecondary onClick={onBack}>
...
export function FinancialProjectionsWizard({ ideaId, idea, onReportReady }) {
  ...
  useEffect(() => {
    ...
    async function load() {
      try {
        // If idea wasn't passed as prop, fetch it
        if (!idea) {
        ...
        // Always try to load saved plan — restore sliders and jump straight to Results
        try {
        ...
        } catch (_) { /* no saved plan — start from Step 1 as normal */ }
      } catch (_) {} finally {
  ...
  return (
    <div>
      ...
      {/* Progress bar */}
      <div className="fp-no-print" style={{ background: '#fff', border: `1px solid ${C.n200}`, borderRadius: '0.875rem', padding: '1.5rem 1.75rem', marginBottom: '1.25rem', boxShadow: '0 1px 3px rgba(0,0,0,0.04)' }}>
      ...
      {stepIdx === 2 && <Step3 inputs={inputs} projections={projections} onBack={goBack} ideaId={ideaId} sectorLabel={sectorLabel} onReportReady={onReportReady} />}
    </div>
  )
}
