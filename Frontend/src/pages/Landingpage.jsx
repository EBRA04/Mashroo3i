          ...
          {navLinks.map(([href, label]) => {
            ...
            return isRoute ? (
              ...
              <a
                ...
                href={href}
                style={sharedStyle}
                onMouseEnter={e=>{ e.target.style.color=C.n900; e.target.style.background=C.n100 }}
                onMouseLeave={e=>{ e.target.style.color=C.n600; e.target.style.background='transparent' }}
              >
                {label}
              </a>
            )
    ...
    <section style={{
      ...
      <div style={{ maxWidth:'780px', margin:'0 auto', position:'relative', zIndex:1 }}>
        ...
        <p style={{ marginTop:'2.75rem', fontSize:'0.875rem', color:'rgba(255,255,255,0.55)' }}>
          Built for entrepreneurs in <strong style={{ color:'#fff', fontWeight:600 }}>Amman, Jordan</strong>
        </p>
      </div>
...
const STATS = [
  ...
  {
    value: 'Local Data',
    label: "Powered by Amman’s latest market trends and industry sectors.",
    icon: (
      <svg width="22" height="22" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round">
...
const FEATURES = [
  ...
  },
  {
    ...
    desc: 'Download a professionally structured PDF business plan tailored to your idea and the Amman market.',
  ...
  },
  {
    ...
    title: 'Amman Market Intelligence',
...
function Footer() {
  return (
    <footer style={{ backgroundColor:'#fff', borderTop:`1px solid ${C.n200}`, padding:'1.5rem 2rem' }}>
      <div style={{ maxWidth:'1200px', margin:'0 auto', display:'flex', flexWrap:'wrap', alignItems:'center', justifyContent:'space-between', gap:'0.75rem' }}>
        <p style={{ fontSize:'0.75rem', color:C.n400, margin:0 }}>
          Mashroo3i benchmarks and evaluations are built for entrepreneurs in Amman, Jordan. Results may not apply to other cities or regions.
        </p>
        <p style={{ fontSize:'0.75rem', color:C.n300, margin:0 }}>
          &copy; {new Date().getFullYear()} Mashroo3i
        </p>
      </div>
    </footer>
  )
